#!/usr/bin/env python3
"""Sinusoidal impedance trajectory with gravity feed-forward."""
...
def main():
    parser = argparse.ArgumentParser(description="Impedance trajectory + gravity demo.")
    ...
    try:
        ...
        while time.time() < end:
            ...
            v = np.zeros(7)
            q_des = q0.copy()
            q_des[1] += args.amplitude * np.sin(2 * np.pi * args.frequency * time.time())
            torque = kp * (q_des - q) - kd * v + gravity_torque(cfg, args.side, q)
