#!/usr/bin/env python3
"""Joint impedance control with Pinocchio gravity feed-forward."""
...
def main():
    parser = argparse.ArgumentParser(description="Joint impedance + gravity compensation demo.")
    ...
    try:
        ...
        q_des = q0.copy()
        q_des[1:3] += 0.1
        print(f"[LiteArmDemo] q_des={np.round(q_des, 4).tolist()}")
        if not args.execute:
